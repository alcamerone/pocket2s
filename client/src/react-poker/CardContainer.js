...
class CardContainer extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (isNewBoardCard || (nextBoard.length === 0 && currBoard.includes(card))) {
      return true;
    } else {
      return false;
    }
  ...
  render() {
    ...
    if (board.includes(cardValue) === true) {
      mapXYZ = dealBoard(cardValue, board, card, size, boardXoffset, boardYoffset);
    }
      ...
      >
        <Motion defaultStyle={{ x: 0, y: 0 }} style={sprungRange}>
          {(
            ...
            <div style={getStyle(x, y, width, height, zIndex)} className="container">
              <Card
                size={size}
                index={index}
